...
def main():
    ...
    if read_result.Ошибка:
        ...
        return 1
    # FAIL "Не заполнен счет дебета" в типовом БП ОбработкаПроведения для возврата
    # сопутки с НДС. Документ создаётся непровёденным с правильными ТЧ. Возможно
    # требуется ДокументОприходования (партия) или принципиально другой режим
    # (не ПокупкаКомиссия). Откладываем.
    res = conn.TL_СопуткаСервис.ОбработатьПакет(read_result.Пакет, False)
