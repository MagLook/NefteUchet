...
Функция СформироватьДеталиСменыСопуткиИзБД(Документ, Заголовок, Режим = "Сопутка") Экспорт
	...
	|	СИ.Тип         КАК Тип,
	|	СИ.ИсточникUUID КАК ИсточникUUID
	...
	Если ВыбС.Следующий() Тогда
		...
		// Флаг «изменён в ЦБ после проведения» (защита проведённых). Чтение безопасно
		// (Попытка внутри) — если ресурса ещё нет в БД, вернётся Ложь.
		ЕстьРасхождениеОсн = TL_МаппингЦБ.ПрочитатьФлагРасхождения(ВыбС.Тип, ВыбС.ИсточникUUID);
	КонецЕсли;
	...
	// Бейдж минусов: если у склада документа есть отрицательные остатки 41.02 — показываем
	// счётчик с классификацией (в скобках !M = требуют действия, непроведённый приход),
	// чтобы при разборе смены бухгалтер видел контекст минусов. Подробности — панель/справка.
	БейджМинусов = "";
	Попытка
		СкладДок = Неопределено;
		Попытка СкладДок = Документ.Склад; Исключение КонецПопытки;
		Если ЗначениеЗаполнено(СкладДок) Тогда
			ТабМин = TL_СопуткаСервис.ДиагностикаОтрицательныхОстатков(СкладДок);
			Если ТабМин.Количество() > 0 Тогда
				ДействийМ = 0;
				Для Каждого Мн Из ТабМин Цикл
					Если Мн.ТребуетДействия Тогда
						ДействийМ = ДействийМ + 1;
					КонецЕсли;
				КонецЦикла;
				БейджМинусов = " &middot; <span class=""route-tag route-transfer"">&#9888; минусы склада: "
					+ XMLСтрока(ТабМин.Количество())
					+ ?(ДействийМ > 0, " (!" + XMLСтрока(ДействийМ) + ")", "") + "</span>";
			КонецЕсли;
		КонецЕсли;
	Исключение
	КонецПопытки;
	...
	HTML = HTML + "<div class=""header""><div>"
		...
		+ ?(ЕстьРасхождениеОсн,
			" &middot; <span class=""route-tag route-transfer"">&#9888; изменён в ЦБ после проведения</span>",
			"")
		+ БейджМинусов
