...
def main():
    ...
    if read_result.Ошибка:
        ...
        return 1
    # Проведение пока FAIL: типовая БП ОбработкаПроведения ВозвратТоваровПоставщику
    # требует доп.настройки для возврата сопутки (СпособУчетаНДС, ДокументОприходования и др.).
    # Документ создаётся непровёденным с правильными ТЧ — ОК.
