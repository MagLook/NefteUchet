...
//   Строка - готовый HTML
//
Функция СформироватьДетальныйОтчётОшибок(ИдентификаторПакета = "", Заголовок = "Журнал ошибок загрузки") Экспорт
	...
	Иначе
		Запрос.Текст =
			...
			|	Т.ЗначениеИсточника, Т.СообщениеОшибки, Т.ВремяРегистрации, Т.Документ
			|ИЗ РегистрСведений.TL_ОшибкиЗагрузки КАК Т
			|ГДЕ Т.ВремяРегистрации >= &Сегодня
	...
	Исключение
		Возврат ОбёрткаHTML("<div style=""padding:20px;color:#888"">Регистр TL_ОшибкиЗагрузки недоступен."
			+ " Возможно, конфигурация не обновлена (UpdateDBCfg).</div>");
	КонецПопытки;
...
// Информационная полоска вверху показывает текущий режим (раздельный учёт).
//
Функция _ДетПак_HTMLЛегендаПроводок(Счёт, РежимРаздельный = Ложь)
	...
	// Информационная полоска про режим
	...
	HTML = HTML + _ДетПак_ЛегендаСтрока("retail_sale_sidegoods", "Отчёт о розничных продажах",
		"Дт 62.Р Кт 90.01.1 (выручка)<br>Дт 90.02.1 Кт 41.02 (с/с)<br>Дт 90.03 Кт 68.02 (НДС)<br>Дт 50.01/57.03 Кт 62.Р (оплаты)",
		Счёт.retail_sale_sidegoods, "", Счёт);
